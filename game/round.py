"""Round class for game state management."""
...
from game.fields import PathField
...
def calculate_wave_money(wave_number: int, base_money: int, bounty_earned: int) -> int:
    ...
    Formula: (base_money + bounty_earned) × (1 + wave_number × 0.15)

    Creates positive feedback loop:
    - Kill enemies → Earn bounties → Fund new towers → Kill more enemies

    Example progression (starting with $450):
    - Wave 1: $450 (baseline)
    - Wave 2: $575 (~+28%)
    - Wave 3: $700 (additional tower funding)
    - Wave 4: $875 (strategic additions possible)
    - Wave 5: $1050 (final tower push)
    """
